...
(function(){
  ...
  function renderAuthor(a){
    const ids=(a.identifiers||[]).map(x=>`<span class="badge">${esc(x.namespace)} ${esc(x.value)}</span>`).join(' ');
    const addresses=(a.addresses||[]).map(x=>esc(x)).join('; ');
    return `<div class="evidence-row"><strong>${a.order_index+1}. ${esc(a.full_name||a.au_name)}</strong> ${ids}${addresses?`<div class="meta">${addresses}</div>`:''}</div>`;
  }
  function renderWosPaper(w){
    ...
    const fundingItems=(funding.items||[]).map(x=>`<div class="evidence-row"><strong>${esc(x.funder)}</strong>${(x.grants||[]).length?` <span class="badge">${esc(x.grants.join(', '))}</span>`:''}</div>`).join('');
    const authorHtml=(r.authors||[]).map(renderAuthor).join('');
    return `<div class="section" id="wos-paper-section">
      ...
      <div class="section"><h3>WoS authors, identifiers & affiliations</h3>${authorHtml||'<div class="muted">No structured author rows.</div>'}</div>
      ${keywordHtml}${classHtml}
      ${funding.funding_agencies_raw?`<div class="section"><h3>Funding</h3>${fundingItems}<div class="evidence-raw">${esc(funding.funding_agencies_raw)}</div>${funding.funding_text_raw?`<div class="evidence-raw">${esc(funding.funding_text_raw)}</div>`:''}</div>`:''}
  ...
  window.showWosReferences=async function(ut){
    try{
      ...
      const rows=(d.items||[]).map(x=>{
        const wos=x.target_in_local_wos?`<span class="badge ok" onclick="event.stopPropagation();showWosRecord('${esc(x.target_ut)}')">local WoS ${esc(x.target_ut)}</span>`:'<span class="badge">external WoS target</span>';
        const zot=x.target_in_zotero?`<span class="badge ok">in Zotero / Paperazzi ${esc((x.target_paper_ids||[]).join(','))}</span>`:'';
        const resolved=x.target_title?`<div class="meta">Resolved target: ${esc(x.target_title)} ${esc(x.target_publication_year||'')}</div>`:'';
        return `<div class="evidence-row"><span class="badge">${x.order_index+1}</span> ${wos} ${zot}${resolved}<div class="evidence-raw">${esc(x.raw_reference)}</div></div>`;
      }).join('');
  ...
  window.showWosCorpus=async function(){
    try{
      ...
      app.innerHTML=`<div class="panel"><div class="toolbar"><strong>WoS Background Corpus</strong><span class="muted">Independent of Zotero</span></div>
        <div class="detail"><div class="grid"><div><div class="stat"><span>WoS records</span><strong>${esc(s.records)}</strong></div><div class="stat"><span>Authors</span><strong>${esc(s.authors)}</strong></div><div class="stat"><span>Cited references</span><strong>${esc(s.cited_references)}</strong></div><div class="stat"><span>Resolved citation edges</span><strong>${esc(s.resolved_citation_edges)}</strong></div></div><div><div class="stat"><span>Zotero active papers</span><strong>${esc(c.active_zotero_papers)}</strong></div><div class="stat"><span>Accepted WoS links</span><strong>${esc(c.matched)}</strong></div><div class="stat"><span>Checked, no local record</span><strong>${esc(c.not_in_local_corpus||0)}</strong></div><div class="stat"><span>Ambiguous</span><strong>${esc(c.ambiguous||0)}</strong></div><div class="stat"><span>Not checked</span><strong>${esc(c.not_checked||0)}</strong></div><div class="stat"><span>Coverage</span><strong>${(Number(c.coverage_fraction||0)*100).toFixed(1)}%</strong></div></div></div>
        <div class="section"><h3>Search the WoS corpus</h3><div class="search"><input id="wos-q" placeholder="Title, DOI, UT, author, ORCID, journal, keyword, institution, funder" onkeydown="if(event.key==='Enter')runWosSearch()"><button onclick="runWosSearch()">Search</button></div><div id="wos-search-results"></div></div>
