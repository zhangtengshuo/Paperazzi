...
async function showIdentityReview(id){
  try{
    ...
    if(d.subject_type==='author'&&d.source_author){
      ...
      const candidateCards=candidates.map((right,index)=>`${authorCompareCard(right,'Candidate '+(index+1))}<div class="review-actions">${right.same_paper_conflict?'<span class="badge warn">Merge blocked: identities co-occur on a paper</span>':`<button class="primary" onclick="mergePair('${esc(left.author_id)}','${esc(right.author_id)}',${id})">Merge source → candidate</button><button class="primary" onclick="mergePair('${esc(right.author_id)}','${esc(left.author_id)}',${id})">Merge candidate → source</button>`}<button onclick="showAuthor('${encodeURIComponent(right.author_id)}')">Open candidate profile</button></div>`).join('');
      app.innerHTML=`<div class="back" onclick="showReviews()">← Identity Review</div><div class="panel detail"><h2>Possible duplicate canonical identities</h2><div class="explain">The queue provides one entry per source identity, while this page lists several similar people. Similarity never performs an automatic merge. Compare every recorded spelling, publications, coauthors and external IDs. Same-paper co-occurrence is shown but cannot be merged.</div>${authorCompareCard(left,'Source identity')}<div class="section"><h3>Similar identities (${candidates.length})</h3>${candidateCards||'<div class="empty">No current similar candidates.</div>'}</div></div>`;
      return;
    }
