...
def _entries_from_matches(normalized: str, matches: list[re.Match[str]]) -> tuple[ReferenceEntry, ...]:
    ...
    for index, match in enumerate(matches):
        ...
        if not raw:
            continue
        entries.append(ReferenceEntry(ordinal=_reference_marker_number(match), raw_text=raw, dois=extract_dois(raw), years=tuple(dict.fromkeys(YEAR_RE.findall(raw)))))
    return tuple(entries)


def segment_reference_entries(text: str) -> tuple[tuple[ReferenceEntry, ...], str, str]:
    normalized = text.replace("\r\n", "\n").replace("\r", "\n")
    for pattern, method in ((NUMBERED_REFERENCE_START_RE, "numbered-punctuated"),(PAREN_NUMBERED_REFERENCE_START_RE, "numbered-parenthesized"),(BARE_NUMBERED_REFERENCE_START_RE, "numbered-bare")):
...
_AUTHOR_LIKE_RE = re.compile(r"\b[A-Z]\.\s*[A-Z][a-z]|\b[A-Z][a-z]+\s+[A-Z]{1,2}\b[,\s]|\bet\s+al\b")
...
def _citation_like_ratio(entries: tuple[ReferenceEntry, ...]) -> float:
    ...
    for entry in sample:
        text = entry.raw_text
        if (re.search(r"\b[A-Z]\.\s*[A-Z][a-z]", text) or re.search(r"\b[A-Z][a-z]+\s+[A-Z]{1,2}\b[,\s]", text) or re.search(r"\bet\s+al\b", text, re.IGNORECASE) or JOURNAL_ABBREV_RE.search(text)):
            hits += 1
    return hits / len(sample)


def prefer_reference_section(primary: ReferenceSection | None, alternate: ReferenceSection | None) -> ReferenceSection | None:
    if primary is None:
        return alternate
...
def _find_implicit_numbered_reference_section(pages: list[str]) -> ReferenceSection | None:
    ...
    for pattern, method in ((NUMBERED_REFERENCE_START_RE, "implicit-numbered-punctuated"),(PAREN_NUMBERED_REFERENCE_START_RE, "implicit-numbered-parenthesized"),(BARE_NUMBERED_REFERENCE_START_RE, "implicit-numbered-bare")):
    ...
    selected_pattern = {"implicit-numbered-punctuated": NUMBERED_REFERENCE_START_RE,"implicit-numbered-parenthesized": PAREN_NUMBERED_REFERENCE_START_RE,"implicit-numbered-bare": BARE_NUMBERED_REFERENCE_START_RE}[method]
    ...
    return ReferenceSection(heading="",start_page=reference_start_page,end_page=len(pages) - 1,method=method,confidence="HIGH",raw_text=raw_text,entries=entries)
...
def find_reference_section(page_texts: Iterable[str]) -> ReferenceSection | None:
    ...
    return ReferenceSection(heading=heading,start_page=page_index,end_page=len(pages) - 1,method=method,confidence=confidence,raw_text=raw_text,entries=entries)
...
def _candidate_spans(blocks_by_page: list[list[tuple[Any, ...]]], *, max_front_pages: int) -> tuple[tuple[EvidenceSpan, ...], tuple[EvidenceSpan, ...], tuple[EvidenceSpan, ...], tuple[str, ...]]:
    ...
    for page_index, blocks in enumerate(blocks_by_page[:max_front_pages]):
        for block in blocks:
            ...
            if any(term in lowered for term in AFFILIATION_TERMS):
                affiliations.append(EvidenceSpan(page_index=span.page_index,text=span.text,kind="affiliation-candidate",bbox=span.bbox))
            ...
            if found_emails or has_correspondence_term:
                correspondence.append(EvidenceSpan(page_index=span.page_index,text=span.text,kind="correspondence-candidate",bbox=span.bbox))
...
def extract_pdf_evidence(path: str | Path, *, max_front_pages: int = 2) -> PdfEvidence:
    ...
    try:
        ...
        return PdfEvidence(path=str(pdf_path),file_size=None,page_count=0,backend="PyMuPDF",backend_version=None,text_status="FILE_UNAVAILABLE",error=str(exc))
    try:
        pymupdf = _load_pymupdf()
    except PdfBackendUnavailable as exc:
        return PdfEvidence(path=str(pdf_path),file_size=file_size,page_count=0,backend="PyMuPDF",backend_version=None,text_status="BACKEND_UNAVAILABLE",error=str(exc))
    ...
    try:
        with pymupdf.open(str(pdf_path)) as doc:
            if getattr(doc, "needs_pass", False):
                return PdfEvidence(path=str(pdf_path),file_size=file_size,page_count=int(getattr(doc, "page_count", 0)),backend="PyMuPDF",backend_version=backend_version,metadata=dict(getattr(doc, "metadata", {}) or {}),text_status="PASSWORD_REQUIRED",error="PDF requires a password")
        ...
        return PdfEvidence(path=str(pdf_path),file_size=file_size,page_count=0,backend="PyMuPDF",backend_version=backend_version,metadata=metadata,text_status="OPEN_OR_PARSE_ERROR",error=f"{type(exc).__name__}: {exc}")
    ...
    front_spans, affiliations, correspondence, emails = _candidate_spans(blocks_by_page,max_front_pages=max_front_pages)
    ...
    if references is not None:
        references = ReferenceSection(heading=references.heading,start_page=references.start_page,end_page=references.end_page,method=references.method,confidence=references.confidence,raw_text=references.raw_text,entries=references.entries,text_channel=("PYMUPDF_SORTED" if references is primary else "PYMUPDF_CONTENT_STREAM"))
    return PdfEvidence(path=str(pdf_path),file_size=file_size,page_count=len(page_texts),backend="PyMuPDF",backend_version=backend_version,metadata=metadata,text_status=_text_status(normal, thin, empty, len(page_texts)),normal_text_pages=normal,thin_text_pages=thin,empty_text_pages=empty,front_matter_text=front_text,front_matter_spans=front_spans,affiliation_candidates=affiliations,correspondence_candidates=correspondence,emails=emails,references=references,error=None)


__all__ = ["EvidenceSpan","PdfBackendUnavailable","PdfEvidence","ReferenceEntry","ReferenceSection","extract_dois","extract_pdf_evidence","find_reference_section","segment_reference_entries"]
