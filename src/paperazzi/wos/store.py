...
class WosCorpusStore:
    ...
    def import_text(
        ...
        *,
        ...
        with self.connect(write=True) as con:
            cur = con.execute(
                "INSERT INTO wos_import_batches(source_filename,source_sha256,label,search_note,imported_at) "
                "VALUES(?,?,?,?,?)",
    ...
    def _upsert_record(
        ...
        record: ParsedWosRecord,
        *,
        batch_id: int,
        imported_at: str,
    ) -> None:
        old = con.execute(
            "SELECT first_imported_at FROM wos_records WHERE ut=?", (record.ut,)
        ).fetchone()
        ...
        con.execute("DELETE FROM wos_addresses WHERE ut=?", (record.ut,))
        con.execute("DELETE FROM wos_organizations WHERE ut=?", (record.ut,))
        con.execute("DELETE FROM wos_correspondence_groups WHERE ut=?", (record.ut,))
        con.execute("DELETE FROM wos_emails WHERE ut=?", (record.ut,))
        con.execute("DELETE FROM wos_keywords WHERE ut=?", (record.ut,))
        con.execute("DELETE FROM wos_classifications WHERE ut=?", (record.ut,))
        con.execute("DELETE FROM wos_funding WHERE ut=?", (record.ut,))
        ...
        for author in record.authors:
            cur = con.execute(
                ...
                (
                    ...
                    author.au_name,
                    author.full_name,
                    normalize_author_key(author.au_name),
                    normalize_author_key(author.full_name),
                ),
            )
            aid = int(cur.lastrowid)
            author_ids[author.order_index] = aid
            au_key = normalize_author_key(author.au_name)
            full_key = normalize_author_key(author.full_name)
            if au_key:
                au_map[au_key] = aid
            if full_key:
                full_map[full_key] = aid
        ...
        for address in record.addresses:
            cur = con.execute(
                ...
                (record.ut, address.order_index, address.address),
            )
        ...
        for i, value in enumerate(record.organizations):
            con.execute(
                "INSERT INTO wos_organizations(ut,order_index,organization) VALUES(?,?,?)",
                (record.ut, i, value),
            )
        ...
        for i, email in enumerate(record.emails):
            con.execute(
                "INSERT INTO wos_emails(ut,order_index,email) VALUES(?,?,?)",
                (record.ut, i, email),
            )
        ...
        for namespace, values in record.classifications.items():
            for i, value in enumerate(values):
                con.execute(
                    ...
                    (record.ut, namespace, i, value),
                )

        if record.funding_agencies or record.funding_text:
            con.execute(
    ...
    def resolve_citation_targets(con: sqlite3.Connection) -> int:
        """Resolve a cited DOI only when it identifies exactly one local WoS record."""
        con.execute(
            """UPDATE wos_cited_references
               SET target_ut = (
                   SELECT MIN(r.ut)
                   FROM wos_records r
                   WHERE r.normalized_doi = wos_cited_references.cited_doi
                   HAVING COUNT(*) = 1
               )
    ...
    def list_references(
        self, ut: str, *, limit: int = 500, offset: int = 0
    ) -> list[dict[str, Any]]:
        with self.connect() as con:
            return [
                dict(row)
    ...
    def citation_frontier(self, *, limit: int = 100) -> list[dict[str, Any]]:
        with self.connect() as con:
            rows = con.execute(
                """SELECT cited_doi,count(*) AS cited_by_count,
                          min(cited_author) AS cited_author,min(cited_year) AS cited_year,
                          min(cited_source) AS cited_source
                   FROM wos_cited_references
                   WHERE cited_doi IS NOT NULL AND target_ut IS NULL
                   GROUP BY cited_doi
                   ORDER BY cited_by_count DESC,cited_doi LIMIT ?""",
